-- Uma transferência validamente criada pode vencer depois que uma das contas
-- foi arquivada. Arquivamento impede novos lançamentos, mas não deve impedir a
-- conclusão/reabertura do histórico já agendado.
...
comment on function public.set_transfer_transaction_status(bigint,text,text,date,uuid) is
  'Conclui ou reabre uma transferencia existente, inclusive quando uma conta foi arquivada depois do agendamento.';
