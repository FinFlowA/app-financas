...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ month?: string }> }) {
  ...
  const [{ data: authData }, accountsResult, transactionsResult, categoriesResult, invoiceItemsResult] = await Promise.all([
    ...
    fetchAllRows((from, to) => supabase
      ...
      .gte("data_compra", `${month}-01`)
      .lt("data_compra", `${nextMonth(month)}-01`)
      .order("id")
      .range(from, to)),
  ]);
  ...
  return <HomeDashboard userId={authData.user.id} month={month} today={hojeEmSaoPaulo()} accounts={(accountsResult.data ?? []) as Conta[]} transactions={(transactionsResult.data ?? []) as Transacao[]} categories={(categoriesResult.data ?? []) as Categoria[]} invoiceItems={(invoiceItemsResult.data ?? []) as FaturaItem[]} />;
}
