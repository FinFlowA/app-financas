...
const INPUT = "mt-1 w-full rounded-ff-sm border border-border bg-surface-muted px-3 py-2.5 font-normal text-foreground outline-none focus:border-primary";
const PAGE_SIZE = 30;
...
function Modal({ title, subtitle, onClose, children, wide = false }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/55 p-0 backdrop-blur-[2px] sm:items-center sm:p-5" onMouseDown={onClose}>
      <section
        role="dialog"
        aria-modal="true"
        aria-label={title}
        className={`max-h-[92dvh] w-full overflow-y-auto rounded-t-ff-lg border border-border bg-surface p-5 shadow-2xl sm:rounded-ff-lg sm:p-6 ${wide ? "sm:max-w-3xl" : "sm:max-w-xl"}`}
        onMouseDown={(event) => event.stopPropagation()}
      >
        <div className="mb-5 flex items-start justify-between gap-4">
          <div><h2 className="text-xl font-extrabold text-foreground">{title}</h2>{subtitle && <p className="mt-1 text-sm text-foreground-muted">{subtitle}</p>}</div>
          <button type="button" onClick={onClose} aria-label="Fechar" className="ff-focus grid h-9 w-9 shrink-0 place-items-center rounded-full bg-surface-muted text-xl font-bold text-foreground-muted">×</button>
        </div>
        {children}
      </section>
    </div>
...
function NewTransactionDialog({ accounts, categories, today, onClose, onChanged }: {
  accounts: Conta[];
  categories: Categoria[];
  today: string;
  onClose: () => void;
  ...
  return (
    <Modal title="Novo lançamento" subtitle="Receita, despesa ou transferência — única, parcelada ou fixa." onClose={onClose} wide>
      <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
        ...
        <fieldset className="sm:col-span-2"><legend className="mb-2 text-sm font-bold">Tipo</legend><div className="grid grid-cols-3 gap-2">
          {(["receita", "despesa", "transferencia"] as const).map((value) => <button key={value} type="button" onClick={() => setKind(value)} className={`rounded-ff-sm border px-2 py-2.5 text-xs font-extrabold sm:text-sm ${kind === value ? value === "receita" ? "border-primary bg-primary-soft text-primary-dark" : value === "despesa" ? "border-red bg-red/10 text-red" : "border-orange bg-orange/10 text-orange" : "border-border bg-surface text-foreground-muted"}`}>{value === "transferencia" ? "Transferência" : value.charAt(0).toUpperCase() + value.slice(1)}</button>)}
        </div></fieldset>
        ...
        <div className="grid gap-3 sm:col-span-2 sm:grid-cols-[1fr_auto_auto] sm:items-center"><Feedback state={feedback} /><button type="button" onClick={onClose} className="rounded-ff-sm border border-border px-5 py-2.5 text-sm font-bold text-foreground-muted">Cancelar</button><button disabled={busy || activeAccounts.length < (kind === "transferencia" ? 2 : 1) || (kind !== "transferencia" && compatibleCategories.length === 0)} className="rounded-ff-sm bg-primary px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">{busy ? "Salvando..." : "Criar lançamento"}</button></div>
      </form>
    </Modal>
  );
}
...
function EditTransactionDialog({ transaction, summary, accounts, categories, userId, onClose, onChanged }: {
  ...
  return <Modal title="Editar lançamento" subtitle={transaction.status === "paga" ? "O status é alterado apenas pelos fluxos Concluir e Reabrir." : "Altere os dados do agendamento com validação de concorrência."} onClose={onClose} wide>
    <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="grid gap-3 sm:col-span-2 sm:grid-cols-[1fr_auto_auto] sm:items-center"><Feedback state={feedback} /><button type="button" onClick={onClose} className="rounded-ff-sm border border-border px-5 py-2.5 text-sm font-bold text-foreground-muted">Cancelar</button><button disabled={busy} className="rounded-ff-sm bg-primary px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">{busy ? "Salvando..." : "Salvar"}</button></div>
    </form>
  </Modal>;
}
...
function CompleteTransactionDialog({ transaction, today, onClose, onChanged }: {
  ...
  return <Modal title="Concluir lançamento" subtitle={`Agendado para ${formatarData(transaction.data_vencimento)}.`} onClose={onClose}>
    <form onSubmit={submit} className="grid gap-4">
      ...
      <div className="grid grid-cols-2 gap-3"><button type="button" onClick={onClose} className="rounded-ff-sm border border-border px-4 py-2.5 text-sm font-bold text-foreground-muted">Cancelar</button><button disabled={busy} className="rounded-ff-sm bg-primary px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">{busy ? "Concluindo..." : "Confirmar"}</button></div>
    </form>
  </Modal>;
}
...
function DeleteTransactionDialog({ transaction, summary, onClose, onChanged }: {
  ...
  return <Modal title="Excluir lançamento" subtitle="Itens concluídos nunca são removidos junto com uma série aberta." onClose={onClose}>
    <form onSubmit={submit} className="grid gap-4">
      ...
      <Feedback state={feedback} />
      <div className="grid grid-cols-2 gap-3"><button type="button" onClick={onClose} className="rounded-ff-sm border border-border px-4 py-2.5 text-sm font-bold text-foreground-muted">Cancelar</button><button disabled={busy} className="rounded-ff-sm bg-red px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">{busy ? "Excluindo..." : "Excluir"}</button></div>
    </form>
  </Modal>;
}
...
function MultiFilter({ label, summary, children, onClear, disabled = false }: {
  ...
  return <details className="group relative" {...(disabled ? { open: false } : {})}><summary className={`ff-focus flex min-h-12 cursor-pointer list-none items-center justify-between gap-3 rounded-ff-sm border border-border bg-surface px-3 py-2 text-sm ${disabled ? "pointer-events-none opacity-45" : ""}`}><span><span className="block text-[10px] font-bold uppercase tracking-wide text-foreground-muted">{label}</span><strong className="block max-w-40 truncate text-foreground">{summary}</strong></span><span className="text-primary group-open:rotate-180">⌄</span></summary><div className="absolute left-0 top-full z-30 mt-2 w-[min(19rem,calc(100vw-2rem))] rounded-ff-md border border-border bg-surface p-3 shadow-xl"><div className="mb-2 flex justify-end"><button type="button" onClick={onClear} className="text-xs font-bold text-primary">Selecionar todos</button></div><div className="max-h-64 space-y-1 overflow-y-auto">{children}</div></div></details>;
}

function FilterOption({ checked, label, color, onChange }: { checked: boolean; label: string; color?: string; onChange: () => void }) {
  return <label className="flex cursor-pointer items-center gap-3 rounded-ff-sm px-2 py-2 hover:bg-surface-muted"><input type="checkbox" checked={checked} onChange={onChange} /><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: color ?? "var(--color-primary)" }} /><span className="min-w-0 flex-1 truncate text-sm font-semibold">{label}</span></label>;
}
...
function TransactionCard({ transaction, summary, accounts, categories, today, onOpen }: {
  ...
  return <article className="ff-card overflow-hidden transition hover:-translate-y-0.5 hover:shadow-md"><button type="button" onClick={onOpen} className="ff-focus grid w-full gap-3 p-4 text-left sm:grid-cols-[64px_minmax(0,1fr)_auto] sm:items-center">
    <div className="hidden h-14 w-14 place-content-center rounded-ff-md bg-surface-muted text-center sm:grid"><span className="text-lg font-black text-foreground">{effectiveDate.slice(8, 10)}</span><span className="text-[10px] font-bold uppercase text-foreground-muted">{new Intl.DateTimeFormat("pt-BR", { month: "short", timeZone: "UTC" }).format(new Date(`${effectiveDate}T12:00:00Z`)).replace(".", "")}</span></div>
...
function InvoiceCard({ invoice, today }: { invoice: InvoiceHistoryGroup; today: string }) {
  ...
  return <article className="ff-card overflow-hidden transition hover:-translate-y-0.5 hover:shadow-md">
...
export default function TransactionManager({ userId, initialMonth, initialQuick, initialOpenNew, today, accounts, categories, cards, invoiceItems, transactions, financialEvents, paymentSummaryRows }: Props) {
  ...
  const counts = Object.fromEntries((["all", "completed", "pending", "overdue", "today", "next7"] as PeriodFilter[]).map((value) => [
    value,
    basicTransactions.filter((transaction) => matchesPeriod(transaction, value)).length
      + basicInvoices.filter((invoice) => matchesInvoicePeriod(invoice, value)).length,
  ...
  function syncUrl(nextPeriod: PeriodFilter, nextMonth = month) {
    ...
    url.searchParams.delete("new");
    if (nextPeriod === "overdue" || nextPeriod === "today" || nextPeriod === "next7") url.searchParams.set("quick", nextPeriod);
  ...
  function changed(message: string) { setNewOpen(false); setEditing(null); setCompleting(null); setDeleting(null); closeDetails(); setFlash(message); router.refresh(); }
  ...
  async function reopen(transaction: TransactionRow) {
    if (operationBusy || !window.confirm(summaryFor(transaction).paymentCount > 0 ? "Reabrir o pagamento mais recente? Os anteriores permanecerão concluídos." : "Reabrir este lançamento?")) return;
  ...
  const periods: { value: PeriodFilter; label: string }[] = [{ value: "all", label: "Todos" }, { value: "completed", label: "Concluídos" }, { value: "pending", label: "Pendentes" }, { value: "overdue", label: "Atrasados" }, { value: "today", label: "Hoje" }, { value: "next7", label: "Próximos 7 dias" }];
  ...
  return <>
    <header className="mb-5 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-wide text-primary">Movimentações</p><h1 className="text-3xl font-extrabold text-foreground">Histórico</h1><p className="mt-1 text-sm text-foreground-muted">Lançamentos, recorrências e pagamentos em um só lugar.</p></div><button type="button" onClick={() => setNewOpen(true)} className="ff-focus rounded-ff-sm bg-primary px-5 py-3 text-sm font-extrabold text-white shadow-md">+ Novo lançamento</button></header>
    ...
    <section className="ff-card mb-4 p-4 sm:p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center justify-center gap-3"><button type="button" onClick={() => changeMonth(-1)} aria-label="Mês anterior" className="ff-focus grid h-10 w-10 place-items-center rounded-full border border-border text-xl font-black text-primary">‹</button><div className="min-w-44 text-center"><p className="text-[10px] font-bold uppercase tracking-widest text-foreground-muted">Período mensal</p><p className="font-extrabold text-foreground">{monthTitle(month)}</p></div><button type="button" onClick={() => changeMonth(1)} aria-label="Próximo mês" className="ff-focus grid h-10 w-10 place-items-center rounded-full border border-border text-xl font-black text-primary">›</button></div><label className="relative block w-full lg:max-w-md"><span className="sr-only">Buscar no Histórico</span><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-foreground-muted">⌕</span><input value={search} onChange={(event) => { setSearch(event.target.value); setLimit(PAGE_SIZE); }} placeholder="Buscar descrição, conta ou categoria" className="w-full rounded-full border border-border bg-surface-muted py-3 pl-10 pr-4 text-sm outline-none focus:border-primary" /></label></div>
      <div className="mt-4 flex gap-2 overflow-x-auto pb-1">{periods.map((item) => <button key={item.value} type="button" onClick={() => choosePeriod(item.value)} className={`shrink-0 rounded-full border px-3 py-2 text-xs font-extrabold ${period === item.value ? "border-primary bg-primary text-white" : "border-border bg-surface text-foreground-muted"}`}>{item.label} <span className={period === item.value ? "text-white/75" : "text-foreground-muted"}>{counts[item.value]}</span></button>)}</div>
      {(period === "overdue" || period === "today" || period === "next7") && <p className="mt-3 text-xs font-semibold text-orange">Este filtro rápido atravessa meses. Use as setas para voltar à visão mensal.</p>}
    ...
    <section className="mb-4 grid grid-cols-3 gap-2 sm:gap-4"><div className="ff-card p-3 sm:p-4"><p className="text-[10px] font-bold uppercase text-foreground-muted">Resultados</p><p className="mt-1 text-xl font-black">{filtered.length}</p></div><div className="ff-card p-3 sm:p-4"><p className="text-[10px] font-bold uppercase text-foreground-muted">Receitas</p><p data-private-value="true" className="mt-1 truncate text-sm font-black text-primary sm:text-xl">{formatarReais(totals.receita)}</p></div><div className="ff-card p-3 sm:p-4"><p className="text-[10px] font-bold uppercase text-foreground-muted">Despesas</p><p data-private-value="true" className="mt-1 truncate text-sm font-black text-red sm:text-xl">{formatarReais(totals.despesa)}</p></div></section>
    <div className="grid gap-3">{filtered.slice(0, limit).map((item) => item.kind === "invoice"
      ...
      : <TransactionCard key={item.key} transaction={item.transaction} summary={summaryFor(item.transaction)} accounts={accounts} categories={categories} today={today} onOpen={() => { void openDetails(item.transaction); }} />)}</div>
    ...
    {newOpen && <NewTransactionDialog accounts={accounts} categories={categories} today={today} onClose={() => setNewOpen(false)} onChanged={changed} />}
    ...
    {detail && detailSummary && <Modal title="Detalhes do lançamento" subtitle={recurrenceLabel(detail.descricao) ?? undefined} onClose={closeDetails} wide>
      <div className="grid gap-5">
        ...
        {!isPagamentoFatura(detail.descricao) ? <div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><button type="button" onClick={() => { const selected = detail; closeDetails(); setEditing(selected); }} className="rounded-ff-sm border border-blue/35 bg-blue/10 px-3 py-2.5 text-sm font-bold text-blue">Editar</button>{!detailSummary.isFullyPaid && <button type="button" onClick={() => { const selected = detail; closeDetails(); setCompleting(selected); }} className="rounded-ff-sm bg-primary px-3 py-2.5 text-sm font-bold text-white">Concluir</button>}{(detailSummary.isFullyPaid || detailSummary.paymentCount > 0) && <button type="button" disabled={operationBusy} onClick={() => { void reopen(detail); }} className="rounded-ff-sm border border-orange/40 bg-orange/10 px-3 py-2.5 text-sm font-bold text-orange disabled:opacity-50">{detailSummary.paymentCount > 0 ? "Reabrir último" : "Reabrir"}</button>}{detailSummary.paymentCount === 0 && <button type="button" onClick={() => { const selected = detail; closeDetails(); setDeleting(selected); }} className="rounded-ff-sm border border-red/40 bg-red/10 px-3 py-2.5 text-sm font-bold text-red">Excluir</button>}</div> : <p className="rounded-ff-sm bg-orange/10 p-3 text-sm font-semibold text-orange">Este item pertence a um pagamento de fatura. Use a tela do cartão para estornar com segurança.</p>}
