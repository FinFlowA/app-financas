...
function EditTransactionDialog({ transaction, summary, accounts, categories, userId, onClose, onChanged }: {
  ...
  return <Modal title="Editar lançamento" subtitle="Altere os dados do agendamento com validação de concorrência." onClose={onClose} wide>
    <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="grid gap-3 sm:col-span-2 sm:grid-cols-[1fr_auto_auto] sm:items-center"><Feedback state={feedback} /><button type="button" onClick={onClose} className="ff-focus rounded-full border border-border px-5 py-3 text-sm font-bold text-foreground-muted transition hover:bg-surface-muted sm:col-start-2">Cancelar</button><button disabled={busy} className="ff-focus rounded-full bg-primary px-6 py-3 text-sm font-extrabold text-white shadow-[0_10px_24px_rgba(22,150,110,0.2)] transition hover:bg-primary-dark disabled:opacity-50 sm:col-start-3">{busy ? "Salvando..." : "Salvar"}</button></div>
    </form>
  </Modal>;
}
...
export default function TransactionManager({ userId, initialMonth, initialQuick, initialOpenNew, initialKind, initialFocusId, returnHomeAfterCreate, today, accounts, goals, categories, cards, invoiceItems, transactions, financialEvents, paymentSummaryRows, reconciledTransactionIds, detailOnly = false, onDetailClosed }: Props) {
  ...
  const openDetails = useCallback(async (transaction: TransactionRow) => {
    ...
    const previewPayments = childPayments.map((item, index) => ({ paymentId: `preview-${item.id}`, paymentSequence: index + 1, transactionId: item.id, value: Number(item.valor), realizationDate: item.data_realizacao ?? item.data_vencimento, adjustmentType: "none" as const, adjustmentValue: 0, active: true, reconciled: reconciledIds.has(item.id), reopenedAt: null, createdAt: null }));
    if (transaction.status === "paga" && summary.paymentCount > previewPayments.length) previewPayments.push({ paymentId: `preview-${transaction.id}`, paymentSequence: summary.paymentCount, transactionId: transaction.id, value: Number(transaction.valor), realizationDate: transaction.data_realizacao ?? transaction.data_vencimento, adjustmentType: "none", adjustmentValue: 0, active: true, reconciled: reconciledIds.has(transaction.id), reopenedAt: null, createdAt: null });
